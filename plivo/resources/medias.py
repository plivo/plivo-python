...
class Medias(PlivoResourceInterface):
    ...
    def upload(self, media_file):
        fileList = []
        if media_file:
            file_extension = media_file.strip().split('.')[-1].lower()
            ...
            for media_url in  media_file:
                fileList.append(('media_file': (media_url.split(os.sep)[-1], open(
                    media_url, 'rb'), content_types[file_extension])))
        else:
            fileList.append(('media_file':''))
        data_to_send = {}
    ...
    def get(self, media_id):
        return self.client.request(
            'GET', ('Media', media_id), response_type=Media)
